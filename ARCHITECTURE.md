...
- Meta upgrades: `features/meta_upgrades/` (`UpgradeManager`, `UpgradeApplier`, `UpgradeMenu`, `UpgradeNode`); `UpgradeDefinition` + effect Resources under `data/upgrades/` (wood batch migrated; towers/buildings/projectiles still enum+match).
